...
1. **Synchronization:**
    ...
    - `/groundstation/toSpaceStation` and `/groundstation/fromSpaceStation` are directories on the Mock Groundstation to send/receieve files with the Mock Spacestation (do not use nested directories)
    - `~/toGroundStation` and `~/fromGroundStation` are directories on the Mock Spacestation to send/receieve files with the Mock Groundstation (do not use nested directories)
...
There are three ways in this repository to experience building an application with all the latency and disconnectedness of space:
...
1. [Deploy to a Linux environment without Visual Studio Code](#Deploy-without-Visual-Studio-Code)
...
1. Ensure provisioning succeeded
    ...
    Once you've SSH connected to the virtual machine, verify everything is configured correctly by inspecting a file we write called `/groundstation/logs/deployGroundStation.log`.
    ...
    ```plaintext
    2021-10-06-202148: Starting Mock SpaceStation Configuration (v 2.1)
    2021-10-06-202148: -----------------------------------------------------------
    2021-10-06-202148: Working Dir: /home/azureuser
    2021-10-06-202148: Installing libraries
    2021-10-06-202148: Deploy Docker in GroundStation (START)
    2021-10-06-202156: Installing regular Docker
    2021-10-06-202221: Docker installed
    2021-10-06-202221: SSH Key Generation in GroundStation (START)
    2021-10-06-202221: SSH Key Generation in GroundStation (COMPLETE)
    2021-10-06-202221: Docker Config (START)
    2021-10-06-202221: Creating docker network 'spaceDevVNet'...
    2021-10-06-202221: Docker network 'spaceDevVNet' created
    2021-10-06-202221: Docker Config (COMPLETE)
    2021-10-06-202221: Building Space Station Image 'mockspacestation-img'...
    2021-10-06-202406: Space Station Image 'mockspacestation-img' successfully built
    2021-10-06-202406: Starting Space Station Container 'mockspacestation'...
    2021-10-06-202408: 'mockspacestation' started...
    2021-10-06-202410: Building SSH connection to 'mockspacestation'...
    2021-10-06-202410: Building sync job to 'mockspacestation' (/groundstation/sync-to-spacestation.sh)...
    2021-10-06-202148: -----------------------------------------------------------
    2021-10-06-202148: Mock SpaceStation Configuration (v 2.1) Complete.  Happy Space Deving!
    ```

    This could take a few minutes, especially at the `SpaceStation Container Build Start` step.

    If you do not see **Mock SpaceStation Configuration (v2.1) Complete** continue to `cat` the file until you see this message.

1. [Validate Mock Spacestion Connection](#Validate-Mock-Spacestation-Connection)

    Once the virtual machine has been deployed and the Mock Spacestation Configuration is complete, get started synchronizing files and ensuring you can connect to the Mock Spacestation following the steps at:
...
### Deploy without Visual Studio Code
1.  ```bash
    curl "https://raw.githubusercontent.com/azure/mock-spacestation/main/.devcontainer/setupScripts/deployGroundStation.sh" \
    && . ./deployGroundStation.sh
    ```
1. Once the containers have been built, get started synchronizing files and ensuring you can connect to the Mock Spacestation following the steps at:
...
1. Write a new message in the `/groundstation/toSpaceStation` directory

    The `/groundstation/toSpaceStation` directory is configured to synchronize to the Mock Spacestation with the latency experienced communicating with the ISS.

    Let's write a file to this directory so we can see it synchronized to the Mock Spacestation:

    ```bash
    echo "Hello! It is currently $(date) on the groundstation! Is anybody home?" >> /groundstation/toSpaceStation/helloFromGroundstation.txt
    ```
...
1. Connect to the Mock Spacestation
    ...
    ```bash
    /groundStation/ssh-to-spacestation.sh
    ```

1. Check the contents of `~/fromGroundStation`

    We should see that file we earlier authored on the Mock Groundstation in the `/groundstation/toSpaceStation` directory. Let's check it out:

    ```bash
    ls ~/fromGroundStation
    ...
    ```bash
    cat ~/fromGroundStation/helloFromGroundstation.txt
    # Hello! It is currently Mon Oct  4 16:20:10 EDT 2021 on the groundstation! Is anybody home?
    ```

1. Write a reply in the `~/toGroundStation` directory:

    The `~/toGroundStation` directory, too, is configured to synchronize to the Mock Groundstation with the latency experienced communicating with the ISS.

    Let's write a file to this directory so we can see it synchronized to the Mock Groundstation:

    ```bash
    echo "Hi there groundstation! It is currently $(date) on the spacestation! The weather is fine." >> ~/toGroundStation/helloFromSpacestation.txt
    ```
...
1. Check the contents of `/groundstation/fromSpaceStation`

    We should see that file we earlier authored on the Mock Spacestation in the `~/toGroundStation` directory. Let's check it out:

    ```bash
    ls /groundstation/fromSpaceStation
    ...
    ```bash
    cat /groundstation/fromSpaceStation/helloFromSpacestation.txt
    # Hi there groundstation! It is currently Mon Oct  4 16:23:05 EDT 2021 on the spacestation! The weather is fine?
    ```

That's it! Any files you place in `/groundstation/toSpaceStation` will synchronize to `~/fromGroundStation` aboard the Mock Spacestation and files placed in `~/toGroundStation` will synchronize to `/groundstation/fromSpaceStation` on the Mock Groundstation!

## Writing your app

Containerize your app and mount the `~/toGroundStation` directory to start synching app output and experiment results to the groundstation.
...
1. Add the docker commands you'll need to run the container to `load-and-start.sh`:
    ...
    docker run -d dotnetsample:latest --volume=/home/azureuser/toGroundStation:/toGroundStation Hello .NET from spacestation.
    ```
...
1. Anything your app writes to `/toGroundStation` will be replicated to `/groundstation/fromSpaceStation`
